...
def test_classify_dirty_files_clean_tree():
    with patch("run_daemon.subprocess.run", return_value=_git_status_mock("")):
        workflow, unknown = _classify_dirty_files("T001")
    assert workflow == []
    assert unknown == []
...
def test_classify_dirty_files_non_runs_files_are_unknown():
    status = " M tools/agent_runner/run_ticket.py\n M README.md\n"
    with patch("run_daemon.subprocess.run", return_value=_git_status_mock(status)):
        workflow, unknown = _classify_dirty_files("T001")
    assert workflow == []
    assert "tools/agent_runner/run_ticket.py" in unknown
    assert "README.md" in unknown
...
def test_classify_dirty_files_mixed():
    ...
    with patch("run_daemon.subprocess.run", return_value=_git_status_mock(status)):
        workflow, unknown = _classify_dirty_files("T001")
    assert "runs/T001/plan.md" in workflow
    assert "tools/run_ticket.py" in unknown
...
def test_ensure_clean_working_tree_unknown_files_aborts():
    with patch("run_daemon._classify_dirty_files", return_value=([], ["tools/run.py"])):
        result = _ensure_clean_working_tree("T001")
