...
# ── PR lifecycle (shared with run_ticket.py) ─────────────────────────────────
import ticket_pr_lifecycle as _ticket_pr_lifecycle  # noqa: E402

_ticket_pr_lifecycle.configure_log(_log)
from ticket_pr_lifecycle import (  # noqa: E402
    _checkpoint_and_push_before_pr,
    _load_state_json as _pr_load_state_json,
    _pr_body,
    auto_merge_pr,
...
def _process_pending_pr_finalization(
    runs_dir: Path,
    worktrees_dir: Path | None,
    repo: str | None,
    dry_run: bool,
) -> None:
    """Finalize TEST_COMPLETE tickets — independent of dispatcher scheduling."""
    for ticket_id, state in scan_tickets(runs_dir, worktrees_dir):
        if state != "TEST_COMPLETE":
            continue
        run_dir = _get_run_dir(ticket_id, runs_dir, worktrees_dir)
        if not needs_pr_finalization(run_dir):
            continue
        worktree_path = worktrees_dir / ticket_id if worktrees_dir else None
        worktree_cwd = str(worktree_path) if worktree_path and worktree_path.exists() else None
        _log(f"{ticket_id}: TEST_COMPLETE pending PR finalization (fallback)")
        if not dry_run:
