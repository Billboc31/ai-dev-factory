...
def is_ignorable_runtime_dirty_path(path: str) -> bool:
    ...
    Covered patterns:
      ...
      - setuptools/pip install metadata: ``*.egg-info/`` / ``*.dist-info/``
    ...
    # Dependency / build output — never auto-commit even if historically tracked.
    # Accidental commits of these paths inflate LLM context and conflict loops.
    ...
    if "target" in parts or "build" in parts or "dist" in parts:
        return True
    if "/.vite/" in path or path.startswith(".vite/") or path.endswith("/.vite"):
        return True

    # Editable-install / wheel metadata generated by pip/setuptools
    if ".egg-info/" in path or path.endswith(".egg-info"):
        return True
    if ".dist-info/" in path or path.endswith(".dist-info"):
        return True
...
def runtime_metadata_dir_root(path: str) -> str | None:
    """Return the setuptools/pip metadata directory root for ``path``, if any."""
    if not path:
        return None
    if path.startswith("./"):
        path = path[2:]
    for sep in (".egg-info/", ".dist-info/"):
        if sep in path:
            return path.split(sep, 1)[0] + sep.rstrip("/")
