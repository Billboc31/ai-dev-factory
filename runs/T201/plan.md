...
Introduce a project-level Execution Rules Engine that evaluates configurable policies against a ticket and emits an `eligible` or `blocked` decision with explicit reasons. The engine is advisory: it persists rule configuration and evaluations and exposes them via the existing Control API and dashboard, but it does not start execution, dispatch workers, reserve workers, reorder queues, or change scheduler/daemon behavior.

## Included

### Database

- Extend `tools/agent_runner/runtime_db.py` (SQLite) and `tools/agent_runner/runtime_db_pg.py` (PostgreSQL) to create the table `project_execution_rules` in both runtime DB initialisation paths, with columns:
...
- Add to `tools/agent_runner/runtime_db.py` and its PG counterpart in `runtime_db_pg.py`:
...
- All helpers accept both SQLite paths and PostgreSQL DSNs via the existing connection abstraction; JSON columns are read back as Python lists/dicts.
...
- `evaluate_ticket(db_path, project_id, ticket_id) -> dict` that:
  ...
  4. Calls `get_execution_approval_state` once and stores the result in `RuleContext.approval_state`.
...
No changes are made to existing scheduler/queue endpoints. No files are created under `tools/api/`.

### Frontend (dashboard)

All UI work targets the existing dashboard under `apps/dashboard/src/` (React + React Router). No file is created under `web/` and no Next.js conventions (`web/app/...`, `pages/` router, `[param]` segments) are used.

- Extend `apps/dashboard/src/api/tickets.js` (or add a sibling file `apps/dashboard/src/api/rules.js` if isolation is preferred) with:
...
- Worker reservation, retry logic, daemon behaviour changes, or any change to the existing execution pipeline.
...
- Any work under `tools/api/` — this path is not used in T201.
- Any work under `web/` or any Next.js conventions (`web/app/...`, `web/lib/...`, `web/components/...`, `[param]` route segments, file-based `pages/` router) — these paths are not used in T201.
- Duplication of approval lifecycle logic inside `execution_rules_engine.py`; the engine reads only the canonical state exposed by `compute_execution_eligibility`.
...
2. Project rules are configurable through `GET` / `PUT /projects/{project_id}/rules` served by `services/control_api/routes/rules.py`, registered in `services/control_api/main.py`, and the stored set is returned merged with registry defaults.
...
11. No file is created under `tools/api/` for T201, and no file is created under `web/` for T201. The new API routes live under `services/control_api/routes/rules.py`, are registered in `services/control_api/main.py`, and their schemas live in `services/control_api/models/schemas.py`. All dashboard work lives under `apps/dashboard/src/` and follows the existing React Router conventions, not Next.js `web/app/...` conventions.
12. The rules engine approval state is resolved exclusively through `get_execution_approval_state`, which wraps `ticket_approval_service.compute_execution_eligibility(...)`; `execution_rules_engine.py` does not read approval tables directly and does not duplicate approval lifecycle logic.
13. The full existing test suite continues to pass alongside the new Python tests under `tests/` and the new dashboard tests under `apps/dashboard/tests/`.
